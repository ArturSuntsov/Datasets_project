...
from apps.datasets_core.views import (
    DatasetCollectionView,
    DatasetDetailView,
    DatasetUploadView,
    DatasetUploadStatusView,
    DatasetDownloadView,
)
...
urlpatterns = [
    ...
    path("api/", include([
        ...
        # Добавить эти строки внутрь path("api/", include([...])):

        # Data Lake endpoints (загрузка/скачивание файлов)
        path("datasets/<str:dataset_id>/upload/", DatasetUploadView.as_view(), name="dataset-upload"),
        path("datasets/<str:dataset_id>/upload-status/", DatasetUploadStatusView.as_view(), name="dataset-upload-status"),
        path("datasets/<str:dataset_id>/download/", DatasetDownloadView.as_view(), name="dataset-download"),
