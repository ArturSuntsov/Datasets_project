...
const siteNavItems: SiteNavItem[] = [
  ...
  { to: "/tasks", label: "Задачи", icon: "✅", roles: ["admin"] },
  {
    to: "/labeling",
    label: "Разметка",
    icon: "🏷️",
...
export default function AnnotationPage() {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      const isGoldenCheck = evaluation?.state === "golden_checked";
      const accessStatus = String(metrics.access_status || "");
      if (
        isGoldenCheck &&
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-gray-100 text-gray-900 dark:bg-gray-950 dark:text-white">
      <header className="relative z-[110] flex min-h-[56px] items-center gap-2 border-b border-gray-200 bg-white px-3 shadow-sm dark:border-gray-800 dark:bg-gray-950">
        ...
        <div className="flex items-center gap-1">
          ...
          <InstructionPanel
            projectId={assignmentQuery.data.project_id}
            bundle={assignmentQuery.data.instructions_bundle}
            fallbackText={assignmentQuery.data.instructions}
            compact
          />
          <button
            type="button"
