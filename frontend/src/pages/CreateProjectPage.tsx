      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,0.75fr]">
          ...
          <div className="card space-y-4">
            ...
            <TextareaField label="Описание" value={description} onChange={setDescription} placeholder="Что нужно сделать и зачем?" rows={3} />
          ...
          <div className="card space-y-4">
            ...
            {needsLabels && (
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Метки (через запятую)</label>
            ...
            {taskConfig?.uses_cv_workflow && <InputField label="Интервал кадров, сек" type="number" step="0.1" value={frameInterval} onChange={setFrameInterval} />}
            ...
            {(annotatorsQuery.data?.items ?? []).map((p: Participant) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1 text-xs text-gray-500">{p.specialization || "Без специализации"} · рейтинг {p.rating?.toFixed(2) ?? "0.00"}</div>
                </button>
              );
            })}
          </div>
