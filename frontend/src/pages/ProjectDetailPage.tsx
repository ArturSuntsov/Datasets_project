...
// =============================================================================
// DAWID-SKENE QUALITY COMPONENT (встроен в этот же файл)
// =============================================================================
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {annotators.map((a) => (
          ...
          >
            ...
            {/* Confusion Matrix */}
            {a.confusion_matrix &&
            Object.keys(a.confusion_matrix).length > 0 ? (
              <div className="mt-3">
                ...
                <table className="w-full text-[11px]">
                  <thead>
                    <tr>
                      ...
                      {Object.keys(
                        Object.values(a.confusion_matrix)[0] || {}
                      ).map((label) => (
                        <th
                          key={label}
                  ...
                  <tbody>
                    {Object.entries(a.confusion_matrix).map(
                      ([trueLabel, row]) => (
                        <tr key={trueLabel}>
...
export default function ProjectDetailPage() {
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      setUploadQueue([]);
      setUploadError(
        result?.asset_status === "failed"
          ? result.error_message || "Video was uploaded, but processing failed."
          : null
      );
  ...
  const finalizeMutation = useMutation({
    mutationFn: async () => {
      if (!projectId || !activeImportId) throw new Error("Nothing to finalize");
  ...
  const exportArchiveMutation = useMutation({
    ...
    onError: (err: any) => {
      setArchiveError(
        err?.response?.data?.detail || err?.message || "Archive export failed"
      );
    },
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            className="btn-secondary"
            onClick={() => exportArchiveMutation.mutate()}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="card">
          ...
          <div className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
            {overview?.imports?.frames_total ?? 0}
          </div>
        </div>
        <div className="card">
          <div className="text-sm text-gray-500 dark:text-gray-400">Work items</div>
          <div className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
            {overview?.work_items?.total ?? 0}
          </div>
        </div>
        <div className="card">
          <div className="text-sm text-gray-500 dark:text-gray-400">
            Low-agreement requeue
          </div>
          <div className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
            {overview?.assignments?.disputed ?? 0}
          </div>
        </div>
        <div className="card">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.2fr,0.8fr]">
        <div className="card space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              Upload images or videos. Videos are split into frames using the
              project frame interval.
            </p>
          </div>
          <input
            type="file"
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              disabled={uploadMutation.isPending || uploadQueue.length === 0}
            >
            ...
            <button
              ...
              disabled={!activeImportId || finalizeMutation.isPending}
            >
            ...
            <select
              ...
              value={exportFormat}
              onChange={(event) =>
                setExportFormat(event.target.value as "coco" | "yolo" | "both")
              }
            >
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-100">
            Assignments for annotators are created only after you click{" "}
            <span className="font-semibold">Finalize import</span>. Until then,
            the project will not appear in their annotation workspace.
          </div>
          {uploadError && (
            ...
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900 dark:border-blue-800 dark:bg-blue-950 dark:text-blue-100">
              ...
              {lastUploadPreview.cleanup && (
                <div className="mt-2">
                  ...
                  {lastUploadPreview.cleanup.duplicates_removed ?? 0}, invalid
                  frames removed{" "}
                  {lastUploadPreview.cleanup.invalid_frames_removed ?? 0}
                </div>
              )}
              {lastUploadPreview.errors.length > 0 && (
                <div className="mt-2">
                  Errors: {lastUploadPreview.errors.join("; ")}
                </div>
              )}
            </div>
          )}
        ...
        <div className="card space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              Live settings for import, routing, AI suggestions, and review quality
              control.
            </p>
          </div>
          <div className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
            <div>
              Labels:{" "}
              {projectQuery.data.label_schema
                .map((item) => item.name)
                .join(", ") || "—"}
            </div>
            <div>Frame interval: {projectQuery.data.frame_interval_sec}s</div>
            <div>
              Annotators per frame: {projectQuery.data.assignments_per_task}
            </div>
            ...
            <div>
              Assignment scope:{" "}
              {String(
                projectQuery.data.participant_rules?.assignment_scope ||
                  "selected_only"
              )}
            </div>
            <div>
              AI pre-labeling:{" "}
              {projectQuery.data.participant_rules?.ai_prelabel_enabled === false
                ? "disabled"
                : "enabled"}
            </div>
            <div>
              AI model:{" "}
              {String(
                projectQuery.data.participant_rules?.ai_model ||
                  "baseline-box-v1"
              )}
            </div>
            <div>
              AI confidence:{" "}
              {String(
                projectQuery.data.participant_rules?.ai_confidence_threshold ??
                  0.7
              )}
            </div>
            <div>
              Keyframe interval:{" "}
              {String(
                projectQuery.data.participant_rules?.video_keyframe_interval ?? 1
              )}
            </div>
            <div>
              Tracking:{" "}
              {String(
                projectQuery.data.participant_rules?.tracking_algorithm || "CSRT"
              )}
            </div>
            <div>
              Task batch size:{" "}
              {String(
                projectQuery.data.participant_rules?.task_batch_size ?? 10
              )}
            </div>
            <div>
              Min consecutive frames:{" "}
              {String(
                projectQuery.data.participant_rules?.min_sequence_size ?? 3
              )}
            </div>
            <div>
              Annotator pool size:{" "}
              {projectQuery.data.allowed_annotator_ids.length}
            </div>
            <div>
              Workflow batches created:{" "}
              {String(overview?.work_items?.workflow_batches_total ?? 0)}
            </div>
            <div>
              Validation-ready frames:{" "}
              {String(overview?.work_items?.validation_ready_items ?? 0)}
            </div>
          </div>
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 text-sm dark:border-gray-800 dark:bg-gray-950">
            ...
            <div className="mt-2 whitespace-pre-wrap text-gray-600 dark:text-gray-400">
              {projectQuery.data.instructions || "No instructions added yet."}
            </div>
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-gray-200 bg-white p-3 text-xs dark:border-gray-800 dark:bg-gray-950">
                <div className="text-gray-600 dark:text-gray-400">
                  ...
                  <a
                    ...
                    href={projectQuery.data.instructions_file_uri}
                    target="_blank"
                    rel="noreferrer"
                  >
                ...
                <div className="text-gray-500 dark:text-gray-400">
                  ...
                  {projectQuery.data.instructions_updated_at
                    ? ` · ${new Date(projectQuery.data.instructions_updated_at).toLocaleString()}`
                    : ""}
                </div>
              </div>
            )}
            {(user?.role === "customer" || user?.role === "admin") && (
              <div className="mt-3 space-y-2">
                <div className="text-xs font-medium text-gray-700 dark:text-gray-300">
                  Загрузить файл инструкции (PDF/DOCX/MD/TXT)
                </div>
                <input
                  type="file"
                  ...
                  onChange={(event) =>
                    setInstructionFile(
                      (event.target.files?.[0] as File | undefined) ?? null
                    )
                  }
                  className="block w-full text-sm text-gray-600 dark:text-gray-300"
                />
                <div className="flex justify-end">
                  <button
                    ...
                    className="btn-secondary"
                    disabled={
                      !instructionFile || instructionUploadMutation.isPending
                    }
                    onClick={() => instructionUploadMutation.mutate()}
                  >
                    {instructionUploadMutation.isPending
                      ? "Загрузка..."
      ...
      {/* ===== Annotator Quality Snapshot (старый overview) ===== */}
      <div className="card">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
            Annotator quality snapshot
          </h2>
              ...
              {(overview?.annotators ?? []).map((annotator) => (
                ...
                >
                  ...
                  <td className="py-2 pr-3">
                    {annotator.rating?.toFixed(2) ?? "0.00"}
                  </td>
                  <td className="py-2 pr-3">{annotator.open_assignments}</td>
                  <td className="py-2 pr-3">
                    {annotator.submitted_assignments}
                  </td>
                  <td className="py-2 pr-3">
                    {annotator.conflict_rate?.toFixed(2) ?? "0.00"}
                  </td>
                </tr>
              ))}
      ...
      <div className="card">
        ...
        <div className="mt-4 space-y-2">
          {(securityEventsQuery.data?.items ?? [])
            .slice(0, 10)
            .map((event) => (
              <div
                key={event.id}
                className="rounded-lg border border-gray-200 p-3 text-xs dark:border-gray-800"
              >
                <div className="flex items-center justify-between">
                ...
                <pre className="mt-2 max-h-24 overflow-auto rounded bg-gray-950 p-2 text-[11px] text-green-200">
      ...
      {exportPayload && (
        <div className="card">
          ...
          <pre className="mt-4 max-h-[420px] overflow-auto rounded-lg bg-gray-950 p-4 text-xs text-green-200">
            {exportPayload}
          </pre>
        </div>
      )}
